...
namespace Sion
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Make the menu
            Config = new Menu("亡灵战神-塞恩", "Sion", true);
            ...
            Config.AddSubMenu(new Menu("大招 设置", "R"));
            Config.SubMenu("R").AddItem(new MenuItem("AntiCamLock", "不锁定 镜头").SetValue(true));
            Config.SubMenu("R").AddItem(new MenuItem("MoveToMouse", "跟随鼠标移动(测试)").SetValue(false));
                //Disabled by default since its not legit Keepo


            Config.AddItem(new MenuItem("yule", "带你装逼带你飞"));

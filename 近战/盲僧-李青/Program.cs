    ...
    internal static class Program
    {
        ...
        public static Vector3 GetInsecPos(Obj_AI_Hero target)
        {
            try
            {
                ...
                if (GetAllyHeroes(target, 1500).Count > 0
                    && ParamBool("ElLeeSin.Insec.Ally"))
                {
                    var insecPosition =
                        InterceptionPoint(
                            GetAllyInsec(
                                GetAllyHeroes(target, 1500)));
        ...
        private static void Combo()
        {
            ...
            {
                if ((castQAgain || spells[Spells.Q].GetDamage(target, 1) > target.Health + target.PhysicalShield)
        ...
        {
            if (!spells[Spells.E].IsReady() || Player.IsDashing() || Player.Spellbook.IsCastingSpell)
            {
                return;
            }
            ...
            {         
                var target =
                    HeroManager.Enemies.Where(
                        x =>
                        x.Distance(ObjectManager.Player) < spells[Spells.E].Range && x.IsValidTarget(spells[Spells.E].Range))
                        .ToList();
            ...
            {
                var target = HeroManager.Enemies.Where(i => HasEBuff(i) && i.IsValidTarget(spells[Spells.E].Range + 145)).ToList();
                if (!target.Any())
                {
                    return;
                }

                if ((PassiveStacks == 0 || target.Count >= 2) || target.Any(t => t.Distance(Player) > Orbwalking.GetRealAutoAttackRange(Player) + 45))
                {
                    spells[Spells.E].Cast();
                }
            }
        ...
        private static void AfterAttack(AttackableUnit unit, AttackableUnit target)
        {
            ...
            if (spells[Spells.E].IsReady() && target.Type == GameObjectType.obj_AI_Hero && EState)
            {
                spells[Spells.E].Cast();
            }

    ...
    public static class Geometry
    {
        ...
        public class Polygon
        {
            ...
            public class Line : Polygon
            {
                ...
                public float Length
                {
                    get { return this.LineStart.Distance(this.LineEnd); }
                    set { this.LineEnd = (this.LineEnd - this.LineStart).Normalized() * value + this.LineStart; }
                }
                ...
                public void UpdatePolygon(int offset = 0, float overrideWidth = -1)
                {
                    ...
                    for (var i = 1; i <= this._quality; i++)
                    {
                        ...
                        var point = new Vector2(
                            this.Center.X + outRadius * (float)Math.Cos(angle), this.Center.Y + outRadius * (float)Math.Sin(angle));
                        this.Points.Add(point);
                    }
                }
            }
            ...
            public class Rectangle : Polygon
            {
                ...
                /// The direction.
                /// </value>
                public Vector2 Direction { get { return (this.End - this.Start).Normalized(); } }
                ...
                /// The perpendicular.
                /// </value>
                public Vector2 Perpendicular { get { return this.Direction.Perpendicular(); } }
                ...
                public void UpdatePolygon(int offset = 0, float overrideWidth = -1)
                {
                    this.Points.Clear();
                    this.Points.Add(
                        this.Start + (overrideWidth > 0 ? overrideWidth : this.Width + offset) * this.Perpendicular - offset * this.Direction);
                    this.Points.Add(
                        this.Start - (overrideWidth > 0 ? overrideWidth : this.Width + offset) * this.Perpendicular - offset * this.Direction);
                    this.Points.Add(
                        this.End - (overrideWidth > 0 ? overrideWidth : this.Width + offset) * this.Perpendicular + offset * this.Direction);
                    this.Points.Add(
                        this.End + (overrideWidth > 0 ? overrideWidth : this.Width + offset) * this.Perpendicular + offset * this.Direction);
                }
            }
            ...
            public class Ring : Polygon
            {
                ...
                {
                    this.Points.Clear();
                    var outRadius = (offset + this.InnerRadius + this.OuterRadius) / (float)Math.Cos(2 * Math.PI / this._quality);
